...
import OrderPatientDetails from './OrderPatientDetails';
...
const OrderPatientTableV2 = () => {
    ...
    useEffect(() => {
        if (error) {
            ...
            handleNonPermissionError(error, 'Không thể tải dữ liệu đơn hàng bệnh nhân.');
            setOrdersData([]);
        } else if (orders) {
    ...
    const handleViewDetail = async (record) => {
        ...
        try {
            ...
            console.error('Failed to fetch patient order details:', error);
            message.error('Không thể lấy chi tiết đơn hàng bệnh nhân!');
        }
    };

    const handleFilterChange = (newFilters) => {
        // Đảm bảo luôn giữ filter isOrderPatient=true
        setFilters({
            ...newFilters,
            isOrderPatient: true
        });
    ...
    return (
        <div className="orders-patient-table-container">
            <PageWrapperV2
                ...
                searchProps={{
                    ...
                    placeholder: 'Tìm kiếm đơn hàng bệnh nhân',
                }}
                filterProps={{
                    filters: filters,
            ...
            >
                <ReusableTableV2
                    ...
                    columns={[
                        ...
                        { dataIndex: 'customerName', title: 'Tên bệnh nhân', render: (text) => text || 'N/A', align: 'center' },
                        {
                            dataIndex: 'orderDate',
                            title: 'Thời gian đặt',
                            render: (date) =>
                        ...
                        { dataIndex: 'area', title: 'Khu vực', render: (text) => text || 'N/A', align: 'center' },
                        { dataIndex: 'location', title: 'Vị trí', render: (text) => text || 'N/A', align: 'center' },
                        {
                            dataIndex: 'status',
                            title: 'Trạng thái',
                            render: (status) => ({
                    ...
                    emptyMessage="Không tìm thấy đơn hàng bệnh nhân nào."
...
export default OrderPatientTableV2;
