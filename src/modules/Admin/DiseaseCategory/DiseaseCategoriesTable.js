...
const DiseaseCategoriesTable = () => {
  ...
  const createDiseaseCategoryMutation = useCreateDiseaseCategory();
  const updateDiseaseCategoryMutation = useUpdateDiseaseCategory();
  const deleteDiseaseCategoryMutation = useDeleteDiseaseCategory();
  ...
  // Handle create or update disease category
  const handleCreateOrUpdate = async (formData) => {
    try {
      const payload = { name: formData.name, branchId };
      if (formData.id) {
        await updateDiseaseCategoryMutation.mutateAsync({ categoryId: formData.id, categoryData: payload, branchId });
        message.success('Cập nhật danh mục bệnh thành công');
        handleEditCancel();
      } else {
        await createDiseaseCategoryMutation.mutateAsync({ categoryData: payload, branchId });
        message.success('Tạo danh mục bệnh thành công');
        handleAddCancel();
      }
      refetch();
    } catch (error) {
      console.error('Failed to save disease category:', error);
  ...
  const handleDelete = async (record) => {
    ...
    try {
      await deleteDiseaseCategoryMutation.mutateAsync({ categoryId: record.id, branchId });
