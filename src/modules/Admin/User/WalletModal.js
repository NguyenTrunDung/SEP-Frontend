...
const WalletModal = ({ visible, onClose, user }) => {
    ...
    const depositColumns = [
        { title: 'Số tiền', dataIndex: 'amount', key: 'amount', render: (v) => format.currency(v) },
        { title: 'Số dư sau', dataIndex: 'balanceAfter', key: 'balanceAfter', render: (v) => format.currency(v) },
        ...
        >
            <div style={{ marginBottom: 16, color: 'red', fontSize: '2em' }}>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#52c41a' }}>
                    {format.currency(user?.balance) || '0'}
                </div>
            </div>
            <Tabs items={[
                {
