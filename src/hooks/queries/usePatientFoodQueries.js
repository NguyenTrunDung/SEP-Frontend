...
import { mockMenus, mockFoodCategories } from '../../mocks/menuData';

// Mock patient service for food-related operations
const mockPatientService = {
  getAllowedFoodsForPatient: async (patientId, diseaseCategoryId) => {
    await new Promise(resolve => setTimeout(resolve, 300)); // Simulate API delay
    return {
      data: diseaseCategoryId
        ? mockMenus.filter(menu => menu.categoryId === diseaseCategoryId)
        : mockMenus
    };
  },

  getRestrictedFoodsForPatient: async (patientId, branchId) => {
    await new Promise(resolve => setTimeout(resolve, 300));
    return {
      data: mockMenus.filter((_, index) => index % 3 === 0) // Every third item restricted
    };
  },

  validateFoodForPatient: async (patientId, foodId, branchId) => {
    await new Promise(resolve => setTimeout(resolve, 200));
    const food = mockMenus.find(menu => menu.ID === foodId);
    return {
      data: {
        foodId,
        isAllowed: food ? Math.random() > 0.3 : false,
        reason: food && Math.random() > 0.3 ? '' : 'Mock restriction for testing'
      }
    };
  },

  validateFoodsForPatient: async (patientId, foodIds, branchId) => {
    await new Promise(resolve => setTimeout(resolve, 400));
    return {
      data: foodIds.map(foodId => ({
        foodId,
        isAllowed: Math.random() > 0.3,
        reason: Math.random() > 0.3 ? '' : 'Mock restriction for testing'
      }))
    };
  },

  getPatientWithDiseaseCategories: async (patientId) => {
    await new Promise(resolve => setTimeout(resolve, 300));
    return {
      data: {
        patientId,
        diseaseCategories: mockFoodCategories.map(category => ({
          id: category.ID,
          name: category.Name
        }))
      }
...
// Mock nurse order service
const mockNurseOrderService = {
  getOrderHistoryForPatient: async (patientId, branchId) => {
  ...
  createOrderForPatient: async (orderData, branchId) => {
    await new Promise(resolve => setTimeout(resolve, 500));
    const food = mockMenus.find(menu => menu.ID === orderData.foodId);
    if (!food) {
      throw new Error('Food item not found');
    }
    if (Math.random() < 0.2) {
      throw new Error(`Food ${food.dishName} is restricted for patient ${orderData.patientId}`);
    }
    return {
      data: {
        orderId: Math.floor(Math.random() * 1000),
...
const patientService = environment.features.enableMockData
  ? mockPatientService
  : require('../../services/patientService').patientService;
const nurseOrderService = environment.features.enableMockData
  ? mockNurseOrderService
  : require('../../services/patientService').nurseOrderService;
